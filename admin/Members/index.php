...
class Admin_Members extends Base_Module {
	...
	*/
	private function listMembers() {
		...
		$query = $this->db->execute('SELECT `id`, `username`, `email` FROM `<ezrpg>players` ORDER BY `id` ASC LIMIT ?,50', array(
			$page * 50
		));
	...
	*/
	private function editMember() {
		...
		$member = $this->db->fetchRow('SELECT `id`, `username`, `email`, `rank`, `money`, `level` FROM `<ezrpg>players` WHERE `id`=?', array(
			intval($_GET['id'])
		));
		...
		if ($errors == 1) {
			...
			$this->tpl->assign('member', $member);
			$this->tpl->assign('MSG_WARN', $msg);
			$this->tpl->display('admin/members_edit.tpl');
			exit;
		} else {
			//No errors, update player info
			$query = $this->db->execute('UPDATE `<ezrpg>players` SET `email`=?, `rank`=?, `money`=?, `level`=? WHERE `id`=?', array(
				$_POST['email'],
				$_POST['rank'],
				$_POST['money'],
				$_POST['level'],
			...
			$msg = 'You have updated the ' . $member->username . '\'s info.';
	...
	*/
	private function deleteMember() {
		$member = $this->db->fetchRow('SELECT `id`, `username` FROM `<ezrpg>players` WHERE `id`=?', array(
			$_GET['id']
		));
		...
		if (!isset($_POST['confirm'])) {
			...
			$query = $this->db->execute('DELETE FROM `<ezrpg>players` WHERE `id`=?', array(
				$member->id
			));
